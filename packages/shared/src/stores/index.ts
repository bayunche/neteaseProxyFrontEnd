...
interface AppState {
  ...
  stats: {
    ...
    realTimeData: RealTimeStatistics;
  };
}
...
const initialState: AppState = {
  ...
  stats: {
    ...
    realTimeData: new StatisticsCollector().getRawData(),
  },
};
    ...
    persist(
      (set, get) => {
        ...
        const audioStateUnsubscribe = audioService.onStateChange((audioState: {
          ...
          const prevSong = currentState.player.currentSong;
          const prevPlaying = currentState.player.isPlaying;
          
          // 统计数据收集逻辑
          if (audioState.currentSong && audioState.isPlaying && !prevPlaying) {
            // 开始播放新歌曲
            currentState.stats.collector.startPlaySession(audioState.currentSong, currentState.player.playMode);
          } else if (audioState.isPlaying && audioState.currentTime > 0 && audioState.duration > 0) {
            // 更新播放进度
            currentState.stats.collector.updatePlayProgress(audioState.currentTime, audioState.duration);
          } else if (prevPlaying && !audioState.isPlaying) {
            // 暂停或停止播放
            currentState.stats.collector.endPlaySession();
          } else if (prevSong && audioState.currentSong && prevSong.id !== audioState.currentSong.id) {
            // 切换歌曲
            currentState.stats.collector.endPlaySession();
          ...
          set((state) => ({
            ...
            stats: {
              ...state.stats,
              realTimeData: state.stats.collector.getRawData()
            }
          }));
        });
        ...
        loadStats: async () => {
          ...
          try {
            // 使用实时统计数据而不是外部服务
            const currentState = get();
            const statsData = currentState.stats.collector.getStatistics();
        ...
        updateStatistics: () => {
          set((state) => ({
            stats: {
              ...state.stats,
              realTimeData: state.stats.collector.getRawData(),
              data: state.stats.collector.getStatistics()
            }
          }));
        },
      ...
      },
      {
        ...
        partialize: (state) => ({
          ...
          // 持久化统计数据
          stats: {
            realTimeData: state.stats.realTimeData
          }
        }),
        ...
        merge: (persistedState: unknown, currentState: AppState) => {
          ...
          if (typedPersisted?.stats?.realTimeData) {
            merged.stats.collector = new StatisticsCollector(typedPersisted.stats.realTimeData);
            merged.stats.realTimeData = typedPersisted.stats.realTimeData;
          }
